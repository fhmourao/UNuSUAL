...
if binsize>1 or binsize<0 or binsize<0.01:
	print "Bin size is out of bounds. Please select a real number from 0 to 1 greater than 0.01."
	exit(0)
...
for i in range(len(user_unexpectedness_metrics)):
	...
	for j in range(len(user_unexpectedness_metrics[i])):
		...
		unexpectedness_intervals[cont_position_interval][j] += len(user_item[i+1])
...
initial_interval = 0
fout.write("Interval M1 M2 M3 M4 M5\n")
maxy = 0
for i in range(len(unexpectedness_intervals)):
...
os.system("gnuplot -e \"datafile='"+output_file+"'; outputname='"+output_file[:-4]+".png'; maxy="+str(maxy+50)+"\" gnuplot_history.gp")